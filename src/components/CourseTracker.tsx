...
import { CheckCircle, Circle, Clock, Calendar, GraduationCap, Share2, Copy, CheckCheck, Target } from 'lucide-react';
...
type SubjectStatus = 'completed' | 'current' | 'planned' | 'available' | 'locked';

const CourseTracker = () => {
  const [completedSubjects, setCompletedSubjects] = useState<Set<string>>(new Set());
  const [currentSubjects, setCurrentSubjects] = useState<Set<string>>(new Set());
  const [plannedSubjects, setPlannedSubjects] = useState<Set<string>>(new Set());
  const [finalizedPeriods, setFinalizedPeriods] = useState<Set<number>>(new Set());
  ...
  const toggleSubjectStatus = (subjectName: string, currentStatus: SubjectStatus) => {
  ...
  const shareProgress = async () => {
    ...
    try {
      if (navigator.share) {
        ...
        toast({
          title: "Link copiado!",
          description: "O link com seu progresso foi copiado para a área de transferência.",
        });
      }
    } catch (error) {
      // Fallback manual
  ...
  const calculateProgress = () => {
    const totalSubjects = data.periodos.reduce((acc, period) => acc + period.disciplinas.length, 0);
  ...
  const estimateGraduation = () => {
    ...
    const [currentYear, currentSemesterNumber] = currentSemester.split('.').map(Number);
    
    // Disciplinas restantes (excluindo as já planejadas para este ano)
    const remainingSubjects = progress.total - progress.completed - progress.current - progress.planned;
    ...
    const remainingSemesters = Math.ceil(remainingSubjects / subjectsPerSemester) + semestersFromPlanned;
  ...
  const getRecommendations = () => {
    ...
    const prioritySubjects = availableSubjects.filter(subject => {
      const dependentCount = data.periodos.reduce((acc, period) => {
        return acc + period.disciplinas.filter(s => 
    ...
    return {
      ...
      other: availableSubjects.filter(s => !prioritySubjects.includes(s)).slice(0, 2)
    };
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">Semestre atual: {graduation.currentSemester}</p>
        </div>
        ...
        <Card className="mb-8">
          <CardContent className="p-4">
            ...
            <div className="mb-4 text-sm text-gray-600">
              <p className="mb-1">• <strong>Primeiro clique</strong> na disciplina: marca como "Planejada para este ano"</p>
              <p className="mb-1">• <strong>Segundo clique</strong> na disciplina: marca como "Cursando"</p>
              <p>• <strong>Terceiro clique</strong> na disciplina: marca como "Concluída"</p>
            </div>
        ...
        <div className="flex justify-center gap-4 mb-8">
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>Tem certeza que deseja limpar todo o progresso?</AlertDialogTitle>
                <AlertDialogDescription>
                  Esta ação é <strong>irreversível</strong>. Todos os dados do seu progresso serão perdidos permanentemente e você terá que preencher tudo novamente do zero.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
        ...
        <div className="grid md:grid-cols-4 gap-6 mb-8">
          <Card>
            <CardContent className="p-6">
              <div className="text-center">
                <div className="text-3xl font-bold text-green-600 mb-2">{progress.percentage}%</div>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-600 mb-2">{graduation.graduationSemester}</div>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="text-center">
                <div className="text-3xl font-bold text-purple-600 mb-2">{progress.current}</div>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="text-center">
                <div className="text-3xl font-bold text-orange-600 mb-2">{progress.planned}</div>
        ...
        <div className="text-center mb-8">
          ...
          >
            ...
            {showRecommendations ? 'Ocultar' : 'Ver'} Recomendações do Próximo Semestre
          </Button>
        </div>
        ...
        {showRecommendations && (
          <Card className="mb-8 border-2 border-blue-200 bg-blue-50">
            <CardHeader>
              <CardTitle className="text-blue-800">Recomendações para o Próximo Semestre</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div>
                  <h4 className="font-semibold text-green-700 mb-2">Disciplinas Prioritárias (desbloqueiam outras):</h4>
                  <div className="grid md:grid-cols-2 gap-2">
                    {recommendations.priority.map((subject, index) => (
                      <Badge key={index} variant="secondary" className="p-2 bg-green-100 text-green-800">
                        {subject.nome}
                      </Badge>
                    ))}
                  </div>
                ...
                {recommendations.other.length > 0 && (
                  <div>
                    <h4 className="font-semibold text-blue-700 mb-2">Outras Opções Disponíveis:</h4>
                    <div className="grid md:grid-cols-2 gap-2">
                      {recommendations.other.map((subject, index) => (
                        <Badge key={index} variant="secondary" className="p-2 bg-blue-100 text-blue-800">
                          {subject.nome}
                        </Badge>
                      ))}
                    </div>
              ...
              <Card key={periodIndex} className="overflow-hidden">
                <CardHeader className="bg-gradient-to-r from-slate-50 to-gray-50">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-lg text-gray-800">{period.periodo}</CardTitle>
                    <Button
                  ...
                  <div className="space-y-3">
                    {period.disciplinas.map((subject, subjectIndex) => {
                      ...
                      return (
                        <div
                          key={subjectIndex}
                          className={`p-3 rounded-lg border-2 transition-all cursor-pointer ${getStatusColor(status)}`}
                          onClick={() => toggleSubjectStatus(subject.nome, status)}
                        >
                            ...
                            <div className="flex-1">
                              ...
                              {subject.prerequisitos && (
                                <div className="text-xs text-gray-500 mt-1">
                                  Pré-requisitos: {subject.prerequisitos.join(', ')}
                                </div>
                              )}
                            </div>
                          </div>
